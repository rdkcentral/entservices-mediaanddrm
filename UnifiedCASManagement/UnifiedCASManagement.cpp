...
// Service registration inside namespace to avoid conflicts
...
const string UnifiedCASManagement::Initialize(PluginHost::IShell* service)
{
    ...
    else
    {
        ...
        message = _T("UnifiedCASManagement plugin could not be initialised");
    }
...
void UnifiedCASManagement::event_data(const string& payload, const string& source)
{
    // Forward the event to JSON-RPC clients through the notification system
    _UnifiedCASManagementNotification.Event_data(payload, source);
}
