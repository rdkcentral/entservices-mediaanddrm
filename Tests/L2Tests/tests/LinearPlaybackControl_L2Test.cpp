...
protected:
    ...
    void SetUp() override {
        // Try to create the expected directory first
        ...
        if (result1 == 0) {
            ...
            // Fall back to creating the expected directory with sudo or alternative approach
            std::cout << "Attempting to create expected directory with alternative method..." << std::endl;
            
            // Try to create /mnt directory and then streamfs
            system("mkdir -p /mnt 2>/dev/null || true");
            int result2 = system("mkdir -p /mnt/streamfs/fcc 2>/dev/null");
            
            if (result2 == 0) {
                mountPoint = expectedPath;
                std::cout << "Successfully created expected mount point: " << mountPoint << std::endl;
            } else {
                // If still failing, use current working directory approach
                char* cwd = getcwd(nullptr, 0);
                if (cwd) {
                    mountPoint = std::string(cwd) + "/test_streamfs";
                    free(cwd);
                } else {
                    mountPoint = "./test_streamfs";
                }
                std::cout << "Using current directory mount point: " << mountPoint << std::endl;
                
                // Create symbolic link to expected location
                std::string linkCmd = "mkdir -p /mnt 2>/dev/null && ln -sf " + mountPoint + " " + expectedPath + " 2>/dev/null || echo 'Link creation failed'";
    ...
    void TearDown() override {
        ...
        // Clean up directories based on what type of mount point we used
        if (mountPoint.find("/mnt/streamfs") == 0) {
            // This is the expected directory, just clean up our files
            std::string cmd = "rm -rf " + fccDir;
            system(cmd.c_str());
        } else {
            // This is an alternative directory, remove it completely
...
TEST_F(LinearPlaybackControlL2Test, GetStatus_Test) {
    ...
    // Use class variable for consistent path and ensure status file exists with proper content
    // Note: Status file does NOT have demux ID suffix (unlike seek0, trick_play0, etc.)
    std::string statusFilePath = fccDir + "/stream_status";
    std::ofstream statusFile(statusFilePath);
    EXPECT_TRUE(statusFile.is_open()) << "Failed to create/update status file: " << statusFilePath;
    statusFile << "0,0"; // 2 values for stream status
    statusFile.close();
    EXPECT_EQ(0, chmod(statusFilePath.c_str(), 0666)) << "Failed to set permissions on status file";
...
TEST_F(LinearPlaybackControlL2Test, GetTracing_Test) {
    ...
    std::cout << "GetTracing_Test: tracing = " << (getResults.HasLabel("tracing") ? (getResults["tracing"].Boolean() ? "true" : "false") : "not found") << std::endl;
