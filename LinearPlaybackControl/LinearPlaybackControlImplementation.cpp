...
namespace WPEFramework
{
    namespace Plugin
    {
        ...
        void LinearPlaybackControlImplementation::dispatchOnSpeedChanged(const int16_t speed, const uint8_t muxId)
        {
            ...
            {
                (*index)->OnSpeedChanged(speed, muxId);
                ++index;
            }
            syslog(LOG_DEBUG, "Dispatched OnSpeedChanged event: speed=%d, muxId=%d", speed, muxId);
        }
        ...
        void LinearPlaybackControlImplementation::speedChangedCallback(const std::string& data)
        {
            ...
            dispatchOnSpeedChanged(trickPlaySpeed, 0);
        }
