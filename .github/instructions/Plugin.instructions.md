...
Plugins using autogenerated JSON-RPC stubs (Exchange::J* classes) must register and unregister them in the Initialize() and Deinitialize() methods. They should not be registered in the constructor or destructor.
...
- If the plugin runs out-of-process, then it should implement the RPC::IRemoteConnection::INotification interface.
